...
def format_reference_output(
  ...
  if relationships:
    ...
    for i, rel in enumerate(relationships[:5], 1):
      ...
      output_lines.append(
        f"**Potential Contribution**: {', '.join(rel.potential_contributions)}"
      )
  ...
  if relevant_refs:
    ...
    output_lines.append("**Reference Function Name Pattern**:")
    ...
    for concept in sorted(list(all_concepts))[:15]:
      output_lines.append(f"0 {concept}")
    output_lines.append("")
...
async def search_code_references(
  ...
  **UNIFIED TOOL**: Search relevant reference code from index files for target file implementation.
  ...
  try:
    ...
    relevent_refs = find_direct_relationships_in_cache(
      target_file, 
      index_cache,
      keyword_list,
      max_results
    ...
    result = {
      ...
      "total_references_found": len(relevent_refs),
    ...
    logger.info(
      f"Successfully found {len(relevent_refs)} references and {len(relationships)} relationships for {target_file}"
    )
